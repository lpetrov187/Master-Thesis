...
- **Known bug (Day 10 full run, 2026-08-12): Tool Selector double-escapes newlines in extracted `code` args.** Confirmed on 2/24 tasks (`code_analysis_04`, `programming_problem_08`) — both show the identical failure signature. The model's JSON output re-encodes a real newline in the source query as the two-character sequence `\n` (literal backslash + n) instead of `\n` (an actual escaped newline), so after `json.loads` the tool receives code with a stray backslash where a line break should be, which Python's parser correctly rejects as "unexpected character after line continuation character." Both tasks show the agent confidently wrong while the baseline was right on the same query. Important nuance: the Claim Verifier is not blind here and did its job correctly (`programming_problem_08` scored groundedness 0.6 and hedged the claims that went beyond the literal evidence) — this is a garbage-in-garbage-out failure one layer upstream of what Day 6/7's mechanisms check (is the answer grounded in the evidence, not whether the evidence itself is accurate). Candidate fix: have the Tool Selector return code as a JSON array of lines instead of a single newline-containing string, sidestepping the escaping issue entirely.
